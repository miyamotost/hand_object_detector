...
def vis_detections_filtered_objects_PIL_custom(im, obj_dets, hand_dets, thresh_hand=0.8, thresh_obj=0.01, font_path='lib/model/utils/times_b.ttf'):
    ...
    if hand_dets is not None:
        im = vis_detections_PIL(im, 'hand', hand_dets, thresh_hand, font_path)
    else:
...
def vis_detections_PIL(im, class_name, dets, thresh=0.8, font_path='lib/model/utils/times_b.ttf'):
    ...
    for hand_idx, i in enumerate(range(np.minimum(10, dets.shape[0]))):
        ...
        state = dets[i, 5]
        if score > thresh:
            image = draw_hand_mask_custom(image, draw, hand_idx, bbox, score, lr, state, width, height, font)

    return image

def draw_hand_mask_custom(image, draw, hand_idx, hand_bbox, hand_score, side, state, width, height, font):
    mask = Image.new('RGBA', (width, height))
    pmask = ImageDraw.Draw(mask)
    pmask.rectangle(hand_bbox, outline=(255, 255, 255), width=4, fill=(255, 255, 255))
